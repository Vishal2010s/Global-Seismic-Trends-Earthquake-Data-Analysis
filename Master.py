...
RAW_CSV = PROJECT_ROOT / "data" / "raw" / "earthquakes1.csv"
FINAL_CSV = PROJECT_ROOT / "data" / "processed" / "EQ_finaldata.csv"
...
def create_database_if_needed():
    """Create the configured MySQL database when it is missing."""
    ...
    if not all([host, user, password, database]):
        raise RuntimeError("MySQL configuration is missing from .env.")
...
def mysql_engine():
    """Create a connection engine for the configured database."""
...
class EarthquakeDataPipeline:
    """Reusable version of the original earthquake data-analysis pipeline."""
    def __init__(self):
        self.raw_csv = RAW_CSV
        self.final_csv = FINAL_CSV

    def data_scraping(self) -> pd.DataFrame:
        ...
        # print(df)
        # print (all_records) 
        print("Data Scrapping completed and saved into csv file")
        return df

    def load_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        #1. 	Load Data
        print("Loading the dataset directly from API or CSV into Pandas")
        df1 = pd.read_csv(self.raw_csv)
        print("Dataset loaded successfully")
        return df1

    def convert_datetime_fields(self, df1: pd.DataFrame) -> pd.DataFrame:
    ...
    def clean_text_fields(self, df1: pd.DataFrame) -> pd.DataFrame:
        # 2.Clean Text Fields 
    ...
    def extract_country_from_place(self, df1: pd.DataFrame) -> pd.DataFrame:
    ...
    def apply_country_replacements(self, df1: pd.DataFrame) -> pd.DataFrame:
    ...
    def normalize_alert_field(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def clean_numeric_fields(self, df1: pd.DataFrame) -> pd.DataFrame:
    ...
    def fill_missing_numeric_values(self, df1: pd.DataFrame) -> pd.DataFrame:
    ...
    def add_derived_columns(self, df1: pd.DataFrame) -> pd.DataFrame:
        ...
        #  pycountry auto-resolves clean country names (203/532 unique values)
        ...
        # Keyword fallback for regions, seas, border areas (329/532 values)
        KEYWORD_MAP = {
            # NORTH AMERICA
        ...
        # ── STEP 3: O(1) dict lookup per row 
        df1['continent'] = df1['country'].map(lookup_cache)
        
        # export as csv
        df1.to_csv(self.final_csv,index=False)
        
        print("Final dataset CSV export completed")
        return df1

    def transfer_to_sql_db(self):
        """Upload the final earthquake CSV to MySQL."""

        print("Starting data upload to SQL database...")

        engine = mysql_engine()
        table_name = "eq_table1"
        ...
        try:
            ...
            print(
                f"Uploaded {len(df2)} rows "
                f"to table {table_name} successfully."
            )

        finally:
            engine.dispose()

    def run(self, upload_to_sql=False):
        """Run the complete earthquake pipeline."""
        """To run the CSV pipeline only:run(self,upload_to_sql=False)"""
        """To create the database when missing and upload the table:run(self,upload_to_sql=True)"""

        df = self.data_scraping()
        df1 = self.load_dataset(df)
        df1 = self.convert_datetime_fields(df1)
        df1 = self.clean_text_fields(df1)
        df1 = self.extract_country_from_place(df1)
        df1 = self.apply_country_replacements(df1)
        df1 = self.normalize_alert_field(df1)
        df1 = self.clean_numeric_fields(df1)
        df1 = self.fill_missing_numeric_values(df1)
        df1 = self.add_derived_columns(df1)

        if upload_to_sql:
            self.transfer_to_sql_db()

        return df1

if __name__ == "__main__":
    pipeline = EarthquakeDataPipeline()
    pipeline.run(upload_to_sql=False)
